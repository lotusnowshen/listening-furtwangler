...
9. 会议记录文件名必须以日期开头，格式为 `YYYY-MM-DD-编号-中文主题名.md`。
10. 日期后必须紧跟两位编号，如 `01`、`02`、`03`。
11. 会议记录文件名优先使用中文，不使用英文 slug，除非用户明确要求英文。
12. 只要用户要求“整理会议记录”“保存会议记录”“导出会议记录”，默认就使用完整记录 / transcript 风格，而不是纪要。
13. 只有用户明确要求“纪要”“摘要”“压缩版”时，才允许改写成纪要。
14. transcript 风格记录应尽量按 `Turn 1`、`Turn 2` 这种逐回合方式整理。
15. transcript 风格记录必须显式保留：
    - 用户指令
    - 主持人每次推进与提示
    - 嘉宾署名发言与回应关系
    - ASCII 框架
    - 中断后继续的说明（如果发生）
16. 除非用户明确要求压缩，不要把多轮圆桌讨论整理成只剩结论的纪要。
17. 版本比较文档默认应保留：
    - 版本基准
    - 各乐章时长
    - 每个乐章的独立比较
    - 每个乐章的聆听要点
    - 如果用户要求，则保留版本 A 时间轴
18. `recording-version-comparison` 一旦被触发，默认就按完整导聆记录文档落地到 `会议记录/`，除非用户明确说不要保存。
19. `会议记录/` 下的每篇文章开头都必须在标题后立刻写清：
    - `作者模型`
    - `模型版本`
    - `写作时间`
20. `作者模型` 默认写为当前实际使用的模型名称。
21. `模型版本` 写当前会话可确认的模型版本字符串，默认直接写简洁值，不额外附带解释性说明。
22. 当前仓库默认写法为：
    - `作者模型：Codex`
    - `模型版本：GPT-5`
23. `写作时间` 至少写到 `YYYY-MM-DD`。
24. 只要用户要求“导赏”“导聆”“详细解析”“具体一些”，就不能只停留在大词层面，必须明确落到：
    - 具体乐器或声部
    - 具体句子或动机
    - 具体段落功能
25. 对多乐章作品，除非用户明确要求压缩，否则每个乐章都应尽量回答清楚：
    - 这段音乐是谁在说话
    - 为什么由这件乐器或这个声部说出来
    - 前后材料如何承接、转向或变形
26. 如果写到“某个旋律很美”“某段很抒情”“某段很高贵”，必须继续往下写清：
    - 美在哪里
    - 是哪件乐器先说出来
    - 这句在结构里到底起什么作用
...
3. `README.md`
4. `prompt.md`
5. `会议记录/2026-03-06-02-富特文格勒战后与维也纳爱乐贝多芬第五交响曲圆桌讨论完整记录.md`
6. `会议记录/2026-03-06-03-萨瓦利什与塞尔舒曼第一交响曲对比导聆记录.md`
7. `会议记录/2026-03-06-04-舒曼第二交响曲详细导聆记录.md`

## Expected Output Style For This Repo

如果本仓库的 `roundtable-seminar` skill 被触发，默认输出应符合以下风格：

- 标题下先写 `作者模型`、`模型版本`、`写作时间`
- 主持人先开场并定义议题
- 明确列出参与人物
- 每位人物署名发言
- 人物之间有直接回应
...
- 每个乐章必须单独拆开比较
- 每个乐章必须保留聆听要点
- 每个乐章的分析不能只停留在抽象判断，必须尽量落到具体乐器、句子、动机和段落功能
- 如果适合，按版本 A 时间轴逐段对照
- 最后给综合判断，而不是只做中性并列描述
